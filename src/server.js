...
const TOOL_DESCRIPTIONS = {
  ...
  query_semantic_model: 'Run a metric query against a context. metrics + group_by + where are validated against the context. Joins are handled for you: group by <relationship>__<attribute> and the declared key is applied — including the validity window of a slowly-changing model, so the attribute is the one valid at the time of each row (no window to state). Pass materialize:true to persist the result and read it back (resilient); slow queries return a query_id to poll.',
