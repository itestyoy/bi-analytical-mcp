...
## Notes
- dbt runs in named environments — one virtualenv each under `/opt/dbt-envs` (`DBT_ENVS_DIR`), picked by `DBT_ENV`: `default` is dbt v2 (`requirements-dbt2.txt`; build arg `INSTALL_DBT_V2=0` makes `default` = `dbt1`), `dbt1` is dbt 1.x + the adapter (build arg `DBT_REQUIREMENTS`: `requirements.txt` for DuckDB, `requirements-bigquery.txt` for BigQuery). MetricFlow is an environment of its own, `metricflow` (build arg `MF_REQUIREMENTS`: `requirements-metricflow.txt` / `requirements-metricflow-bigquery.txt`; `MF_ENV` names another), which every dbt environment queries metrics through. `docker-compose.yml` runs `default`, the BigQuery setup `dbt1` until the python stage is proven on v2 there. The server reads the dbt version from the binary (`DBT_VERSION` pins it; `DBT_BIN`/`MF_BIN` name binaries outright). On v2 the semantic layer is written in dbt's latest YAML spec, and the python stage is not offered on DuckDB. v2 downloads its ADBC driver from dbt's CDN on the first run — allow that once, or warm it at build time.
