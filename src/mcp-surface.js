...
RESULT CARDS: in a host that renders MCP Apps, the results of query_semantic_model, get_query_result and experiment are drawn for the person as cards — a chart (a time series or a breakdown), a funnel (ordered steps), the A/B test, the split check, the sample-size plan. build_native_model draws none: after materialize, call get_query_result({ context_id, table }) on the pipeline's table (its response names it under show_to_user) to show a funnel or a chart, and do not draw your own chart of the same rows. A funnel card is drawn only when the steps are recognisable, so shape a funnel's last stage for it: ONE row whose count columns are named in step order with the word step (step1_install, step2_level_started, …), or a row per step with a column named step; the counts must not grow from one step to the next.
