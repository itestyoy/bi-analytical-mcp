...
шаблона, заполненные, и на них гоняются 51 сценарий в
...
### А если владелец всё-таки нужен

Всё выше исходило из того, что воронку не «владеет» никто: несколько событий делят один id,
значит ни одна сторона по нему не уникальна. Но склад может быть смоделирован наоборот —
**одна строка на воронку** на стороне падений и много событий, ссылающихся на неё. Тогда
источник падений и есть цель соединения, и он это объявляет:

```yaml
# источник падений — ВЛАДЕЛЕЦ
entities:
  ad_funnel: { type: unique, key: [tracking_id, internal_player_id] }

# источник событий — ссылается
entities:
  ad_funnel: { type: foreign, key: [tracking_id_of_event_data, internal_player_id] }
```
...
> владельца в действительности не уникален, соединение размножит строки, и метрика вырастет
> молча. В фикстуре это специально так: четыре отчёта u1 называют одну воронку, и тот же
> запрос даёт **190** вместо честных **184** (сценарии 50-51). Прежде чем ставить `unique`,
> проверьте на складе:
> ```sql
> SELECT tracking_id, internal_player_id, count(*)
> FROM fct_stability_events GROUP BY 1, 2 HAVING count(*) > 1;
> ```
> Пусто — `unique` правда. Не пусто — оставляйте `foreign` с обеих сторон и соединяйте в
> pipeline.
...
| `test/integration/mcp-end-to-end.test.js` | 10 сквозных сценариев целиком через настоящий MCP: discovery → метрика, цепочка на четыре источника, окно валидности, форматы рекламы, контракт `attrs`, воронка со срезом, три источника на `metric_time`, значимость по агрегатам из склада, перенарезка сохранённого результата, расширение задачи |
