...
- recipes: ready-made templates for common task families (trends, segmentation, funnels, retention, cohorts, behavioral, conversion, progression, monetization, ads, economy, stickiness) plus, where python models run, one family that is NOT a business task — bigframes: one recipe per APPROACH (the correct form of a single move on the frame, with the form that raises beside it), which is what a python stage is adapted from. The semantic_index overview lists all their ids; semantic_index({ recipe: id }) returns one in full, semantic_index({ guide: true }) groups them under the tasks families.
