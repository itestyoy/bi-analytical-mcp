...
In a host that renders MCP Apps, a result can be drawn for the person as a card — when the person asks to see it, or when its shape says more as a picture than as a sentence: a trend over time, a comparison across several groups, a funnel, a flow, a drill-down pivot, an A/B test's intervals. A single number, a row or two, a verdict or a plan is answered in words: read the numbers and write the conclusion; a drawing call for such a result answers drawn: false and leaves it to your words. A model result is drawn by one tool, display_model_result({ task_id, display }), after reading it with its query tool (reads draw nothing); it draws a task once — a second call is refused — so one question gets at most one card, and the card is the chart, so do not draw your own of the same rows. An experiment has no task: it draws its own card only with card: true. In \`display\`, pick the \`kind\` whose schema description matches the question; it names result columns and changes no numbers. A pivot, or a chart with drill, reads a stored result: run the query with materialize:true (a pipeline build is stored already).`;
...
function coreInstructions({ apps = false, skillUris = [], featureLines = [] } = {}) {
  return [
    ...
    ...featureLines,
    ...(apps ? ['Draw a card only when the person asks to see a result or its shape is visual (a trend, a funnel, several groups); otherwise answer in words (see RESULT CARDS below).'] : []),
