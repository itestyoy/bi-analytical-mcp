...
RESULT CARDS: in a host that renders MCP Apps, the results of query_semantic_model, get_query_result and experiment are drawn for the person as cards — a chart (a time series or a breakdown), a funnel (ordered steps), the A/B test, the split check, the sample-size plan. A query that outlasts its call answers with a query_id; its card follows the query and draws the result in place when it is ready (where the host lets a card read its result), but YOU still need the rows: call get_query_result({ query_id }) until it is ready before you report the numbers. build_native_model draws none: after materialize, call get_query_result({ context_id, table }) on the pipeline's table (its response names it under show_to_user) to show a funnel or a chart, and do not draw your own chart of the same rows. DECLARE the card with \`display\` (on query_semantic_model or get_query_result) whenever the result is a funnel or a chart, and choose the form by the question — the card then draws exactly that, in the declared order, instead of guessing from column names: a trend → { kind: 'line', x, y: [..], series_column? } (several y, or a series_column, is a multi-line); a total split into parts over time → { kind: 'area', … } (stacked, same fields as line); a comparison across categories → { kind: 'bar', x, y (one or several), series_column?, stacked?, horizontal? }; shares of one total, 3–6 clearly different parts → { kind: 'pie', label_column, value_column } (close values → a bar); ordered steps → { kind: 'funnel', steps: [{ column, label }, …] } for step counts in the columns of ONE row, or { kind: 'funnel', label_column, value_column } for a row per step; headline numbers → { kind: 'kpi', values: [{ column, label?, format? (number|percent|currency), previous_column?, good? (up|down) }], x? } — 1–4 tiles from ONE row, or with x the last row, its change and a sparkline (a single number beats any chart); flows between stages → { kind: 'sankey', source_column, target_column, value_column } (one row per link, no cycles). Each takes an optional title. It names result columns and changes no numbers; a column that is not in the result is refused with the list. Without it, a funnel is only inferred from step-like names (step1_…, a column named step) with counts that do not grow.
 ...
 * only; get_query_result also to the view, which follows its own detached query), and for a tool
 * whose result has an Apps view, the view — as the official ext-apps
