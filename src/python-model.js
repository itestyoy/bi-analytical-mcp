...
export function frameProfile(rt, config = {}) {
  ...
  if (runtime === 'bigquery' && (method === 'bigframes' || !method)) {
    return {
      ...
      guide: 'RULES FOR BIGFRAMES: (1) modelling = bigframes.ml (import { package: "bigframes", submodule: "ml.cluster", names: ["KMeans"] } etc.) — NEVER sklearn here: it needs df.to_pandas() and runs single-node in the notebook; (2) stay vectorized — column expressions, groupby/agg, merge, rolling/window, .str / .dt accessors, Series.map with a dict; AVOID iterrows and df.apply / Series.map with a Python function: BigFrames turns such a function into a BigQuery remote function on Cloud Run (a deploy at call time, extra IAM, latency, cost) — reach for it only when no column expression can say it; (3) execution is deferred — nothing runs until the frame the model returns is materialized; df.cache() only for an expensive intermediate used twice (it stores a temporary BigQuery table you pay for); (4) never rely on row order (partial ordering may be on) — sort_values() explicitly where order matters; (5) df.to_pandas() pulls the whole table into the notebook runtime — only for a small, already-aggregated frame.',
