...
RESULT CARDS: in a host that renders MCP Apps, the results of query_semantic_model, get_query_result and experiment are drawn for the person as cards — a chart (a time series or a breakdown), a funnel (ordered steps), the A/B test, the split check, the sample-size plan. A query that outlasts its call answers with a query_id, and that answer's card only says it moved to the background — it never updates; call get_query_result({ query_id }) until it is ready, which draws the result's card, before you report the numbers. build_native_model draws none: after materialize, call get_query_result({ context_id, table }) on the pipeline's table (its response names it under show_to_user) to show a funnel or a chart, and do not draw your own chart of the same rows. DECLARE the card with \`display\` (on query_semantic_model or get_query_result) whenever the result is a funnel or a chart — the card then draws exactly that, in the declared order, instead of guessing from column names: { kind: 'funnel', steps: [{ column, label }, …] } for step counts in the columns of ONE row, { kind: 'funnel', label_column, value_column } for a row per step, { kind: 'line', x, y: [..], series_column? }, { kind: 'bar', x, y }, each with an optional title. It names result columns and changes no numbers; a column that is not in the result is refused with the list. Without it, a funnel is only inferred from step-like names (step1_…, a column named step) with counts that do not grow.
