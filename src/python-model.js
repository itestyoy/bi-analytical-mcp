  ...
  if (runtime === 'bigquery' && (method === 'bigframes' || !method)) {
    return {
      ...
      runHints: [
        ...
        { match: "unexpected keyword argument|__init__\\(\\) got an unexpected", hint: 'About this runtime: bigframes.ml wears the scikit-learn API but WRAPS BigQuery ML — each constructor takes only the options its BQML model type has, under BQML\'s names, and everything after the first parameter is keyword-only. An argument scikit-learn has and BQML does not is this TypeError (observed: KMeans(standardize_features=True); the same goes for n_init, random_state, algorithm). The signature of every estimator in this version is one fetch away — semantic_index({ recipe: "bf_ml_signatures" }) — and for KMeans it is (n_clusters, *, init, init_col, distance_type, max_iter, tol, warm_start). Scaling is not a flag: preprocessing.StandardScaler() (no parameters), alone or as the first of pipeline.Pipeline\'s exactly two steps, or done in a SQL stage before this one.' },
