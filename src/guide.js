...
export function buildGuide(catalog, recipes, { task, python = null } = {}) {
  ...
  const workflow = [
    ...
    'READ TASKS: create_semantic_model, query_semantic_model and a pipeline materialize return only a task_id and never wait — get_task_result({ task_id }) waits for the task and returns its result (call it again while it says running). Issue independent queries first, then read them.',
  ...
  const routing_triggers = [
    ...
    { if: 'you want to look at a STORED result another way (filter it, regroup it, join an attribute onto it) — a materialized query or a built pipeline', do: 'start a pipeline FROM its task: build_native_model({ action: "start", name, from_task: "<task_id>" }) — its steps read that table, nothing before it is recomputed. Page the stored rows themselves with get_task_result({ task_id, offset, limit }).' },
    { if: 'you only need a QUICK directional read on large data (shape, not an exact number)', do: 'add a `sample` stage to a build_native_model pipeline — a fast ~N% random subset. The result is flagged `approximate`; ALWAYS re-run WITHOUT the sample for any number you will act on (sampling error flips rates near 0/1, small segments, distinct counts).' },
