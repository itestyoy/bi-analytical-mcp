...
function memorySchema() {
  ...
  return {
    type: 'object', additionalProperties: false, required: ['action'],
    description: 'DURABLE analyst memory: save what you FOUND OUT — a vague request tracked down to a real field, a non-obvious gotcha, a useful source — and LINK it to the catalog entities it concerns, so it comes back THROUGH semantic_index next time. Use action:"record" AFTER you have figured something out (e.g. the user said "ad format" and you established it is the property ad_type_of_event_data on ad_started/ad_finished): pass the finding as `note`, the entities it is about as `targets` (a property/attribute/event/model — "ad_type_of_event_data", "users.country", "ad_finished", "users"), the words the user actually used as `aliases` ("ad format"), and any sources as `links`. The note then surfaces on the linked semantic_index views ({ model }/{ event }/{ property }) and via semantic_index({ search }) — so the next fuzzy phrasing resolves straight to the right field. action:"list" (all, or one { target }) / "search" (by word) / "forget" (by id) manage them.',
    allOf: [
      { if: { properties: { action: { const: 'record' } }, required: ['action'] }, then: { required: ['note'], ...forbid(['query', 'id', 'target']) } },
      { if: { properties: { action: { const: 'list' } }, required: ['action'] }, then: forbid(['note', 'targets', 'aliases', 'links', 'query', 'id']) },
      { if: { properties: { action: { const: 'search' } }, required: ['action'] }, then: { required: ['query'], ...forbid(['note', 'targets', 'aliases', 'links', 'id', 'target']) } },
      { if: { properties: { action: { const: 'forget' } }, required: ['action'] }, then: { required: ['id'], ...forbid(['note', 'targets', 'aliases', 'links', 'query', 'target']) } },
    ],
    properties: {
      action: { enum: ['record', 'list', 'search', 'forget'], description: 'record → save a finding; list → all notes (or those linked to one { target }); search → notes matching a word (text/alias/target); forget → delete one note by id.' },
      note: { type: 'string', minLength: 1, description: 'record: the finding to remember, in plain words (e.g. "\'ad format\' = the event_data property ad_type_of_event_data, populated only on ad_started/ad_finished; values rewarded/interstitial/banner").' },
      ...
      query: { type: 'string', description: 'search: a word to match against note text, aliases and linked targets.' },
