...
именем ниже по цепочке не адресуются. Такая **не пропускается молча** — ответ шага несёт
поле `columns_not_added`, где на каждую колонку сказано, **почему** её нет и **чем именно**
её добрать. Два случая там намеренно разведены, потому что означают противоположное:

| случай | что это значит |
|---|---|
| колонка **ключа связи** (`join_key: true`) | значение на обеих сторонах одинаковое по построению — не потеряно ничего, добирать нечего |
| любое **другое** совпавшее имя | на сторонах **разные данные** (в строке падения `app_version` — версия, которая упала; в записи об установке — версия на момент установки). Конвейер оставляет свою, присоединённой в результате нет, пока её не попросят |

Отдельная рекомендация называет только второй случай — тот, который реально стоит данных, —
и сразу отдаёт готовую вставку:

```json
{ "stage": "join", "with": "events", "via": "ad_funnel_rewarded",
  "attrs": ["event_id", { "column": "event_name", "as": "events_event_name" }] }
```

То же сообщение приходит и когда конвейер передан целиком (`register_native_model`,
в том числе `dry_run`) — там оно указывает номер шага. `attrs` сверяется с реальными
колонками присоединённой модели, так что опечатка падает здесь со списком доступного,
а не ошибкой базы.
