...
2. create_semantic_model — declare measures/dimensions/metrics for a task in an ISOLATED context (returns a context_id). Pass that context_id back to extend the same context.
   ...
   - Beyond SQL (z-scores, clustering, scoring, forecasting), where the overview's python_models says available: add a 'python' stage as the LAST stage of that pipeline — declare allowlisted imports, your own functions (def f(df, …): … return df) over the frame dbt.ref() returns on this warehouse (see the stage's 'frame' field: native BigFrames/Snowpark/PySpark by default, pandas only on request) and the ordered calls; the server writes the dbt Python model (dbt.ref/dbt.config/return) and dbt runs it on the warehouse's Python runtime. Read the result with get_query_result as usual.
