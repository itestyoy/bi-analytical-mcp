...
function memorySchema() {
  ...
  return {
    ...
    allOf: [
      ...
      { if: { properties: { action: { const: 'list' } }, required: ['action'] }, then: forbid(['note', 'targets', 'aliases', 'links', 'query', 'id', 'fuzzy']) },
      { if: { properties: { action: { const: 'search' } }, required: ['action'] }, then: { required: ['query'], ...forbid(['note', 'targets', 'aliases', 'links', 'id', 'target']) } },
      { if: { properties: { action: { const: 'forget' } }, required: ['action'] }, then: { required: ['id'], ...forbid(['note', 'targets', 'aliases', 'links', 'query', 'target', 'fuzzy']) } },
    ],
    properties: {
      ...
      query: { type: 'string', description: 'search: a word to match against note text, aliases and linked targets. FUZZY by default — typo- and paraphrase-tolerant (exact-substring hits rank first, then approximate).' },
      fuzzy: { type: 'boolean', description: 'search: enable typo/approximate matching (default true). false = exact substring only.' },
