...
export function buildGuide(catalog, recipes, { task, python = null } = {}) {
  ...
  const workflow = [
    ...
    'READ TASKS: build_semantic_model, query_semantic_model, a pipeline materialize and query_pipeline_model return only a task_id and never wait — the query tool of the same side reads it back: query_semantic_model({ task_id }) for a semantic model or a metric query, query_pipeline_model({ task_id }) for a pipeline build or a query over one. It waits for the task and returns its result (call it again while it says running). Several independent queries on one context go in ONE call — queries: [ … ] (up to 5, run side by side) — and are read back together with { task_ids }.',
