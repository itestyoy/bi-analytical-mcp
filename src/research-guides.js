...
// test/unit/research-guides.test.js holds each recipe id named here to that file). A recipe is
// always written `recipe "<id>"`, so every reference is one pattern the test can find.
//
...
const RESEARCH = {
  ...
  checks: [
    'Row counts after every join: a join that multiplies rows inflates every sum after it — count entities with count_distinct, and join a slowly-changing model point-in-time (recipe "pipeline_point_in_time_join").',
    'No average of averages: compute a rate from summed numerators and denominators (recipe "ratio_metric"), not by averaging per-segment rates.',
    ...
    'Multiple comparisons: with many segments, one will look significant by chance. experiment\'s `correction` (holm, or bh for a false-discovery rate) adjusts the p-values across the variants of one analyze call; a single significant segment among many is a hypothesis to confirm on fresh data, not a finding.',
...
const PRODUCT = {
  ...
  sequence: [
    ...
    'Check tracking by app version and platform over the period (recipe "pipeline_volume_and_coverage_check") before reading behaviour.',
    ...
    'Localise with a funnel over the first session / onboarding / the core loop, per segment (recipe "funnel_from_event_property_steps", recipe "pipeline_ordered_sequence").',
    'Test: an experiment or a staged rollout; otherwise state the self-selection risk.',
  ],
  metrics: [
    { metric: 'N-day retention ("return on day N")', definition: 'Share of an install cohort active on exactly day N after install (D1, D3, D7, D14, D30). The standard for games. Build it on two time axes — the cohort\'s install date and the activity date (recipe "cohort_grid_two_time_axes"; an age axis: recipe "pipeline_age_offset_axis").' },
...
const MONETIZATION = {
  ...
  metrics: [
    ...
    { metric: 'Cumulative ARPU by cohort day (empirical LTV)', definition: 'Revenue to date of an install cohort ÷ its installs, at day 0, 1, 7, 30… — compare cohorts at the same age (recipe "cohort_grid_two_time_axes", recipe "cumulative_metric").' },
...
const UA = {
  ...
  sequence: [
    'Join spend and installs at the same grain (day × network × campaign × country × platform) before bringing revenue in: spend joined to user or event rows multiplies it. Aggregate each source to that grain first (recipe "metrics_from_two_sources" for side by side; a pipeline join by the declared relationship only to carry an attribute).',
...
/** The domain guides, by name — derived from the set, so a new domain is listed everywhere at once. */
export const RESEARCH_DOMAINS = Object.keys(RESEARCH_GUIDES).filter((k) => k !== 'research');

/**
 * THE ONE ROUTING LINE — what an open question is, and where to read how to research it. The
 * guide's trigger, the semantic_index description, the schema's `guide` field, the core
...
export function isResearchGuide(name) {
  ...
  return n === 'research' || n.startsWith('research/');
}

/**
 * The research guide `name` as semantic_index returns it — the one object the tool and the skill
 * both render. An unknown one is refused, naming the ones that exist.
 */
export function researchGuide(name) {
  const key = String(name).trim().toLowerCase();
  const g = RESEARCH_GUIDES[key];
  if (!g) throw new ToolError(`no research guide '${name}' — the research guides are: ${Object.keys(RESEARCH_GUIDES).join(', ')}`, { stage: 'validate', field: 'guide' });
  return {
    task: key,
    ...g,
    ...(key === 'research'
