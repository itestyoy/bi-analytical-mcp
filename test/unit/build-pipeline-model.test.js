...
test('build_pipeline_model: add_step propagates columns; bad step is rejected without mutating the draft', async () => {
  const e = engine();
  const s = await e.build_pipeline_model({ action: 'start', name: 'inc', source: 'events' });
  const a1 = await e.build_pipeline_model({ action: 'add_step', draft_id: s.draft_id, stage: mr, include_columns: true });
  ...
  await assert.rejects(() => e.build_pipeline_model({ action: 'add_step', draft_id: s.draft_id, stage: { stage: 'project', columns: ['no_such_col'] } }));
...
test('build_pipeline_model: add_step returns only the applied step by default; include_steps:true gives the full list', async () => {
  const e = engine();
  ...
  const a3 = await e.build_pipeline_model({ action: 'add_step', draft_id: s.draft_id, stage: { stage: 'order_by', keys: [{ key: 'n', direction: 'desc' }] }, include_steps: true });
  ...
  const ed = await e.build_pipeline_model({ action: 'edit_step', draft_id: s.draft_id, index: 1, stage: { stage: 'where', conditions: [{ column: 'event_name', op: 'eq', value: 'iap_purchase_failed' }] } });
...
test('build_pipeline_model: add_step returns a column diff by default', async () => {
  const e = engine();
  const s = await e.build_pipeline_model({ action: 'start', name: 'diff', source: 'events' });
  const a1 = await e.build_pipeline_model({ action: 'add_step', draft_id: s.draft_id, stage: mr });
...
test('build_pipeline_model: discard drops the draft; later actions error cleanly', async () => {
  const e = engine();
  const s = await e.build_pipeline_model({ action: 'start', name: 'inc', source: 'events' });
  await e.build_pipeline_model({ action: 'add_step', draft_id: s.draft_id, stage: mr });
  const d = await e.build_pipeline_model({ action: 'discard', draft_id: s.draft_id });
  assert.equal(d.discarded, true);
  await assert.rejects(() => e.build_pipeline_model({ action: 'add_step', draft_id: s.draft_id, stage: mr }), /no draft/i);
  await assert.rejects(() => e.build_pipeline_model({ action: 'preview', draft_id: s.draft_id }), /no draft/i);
});
...
test('build_pipeline_model: starting again in the same context resets the draft', async () => {
  ...
  const s2 = await e.build_pipeline_model({ action: 'start', draft_id: s.draft_id, name: 'second', source: 'events' });
...
test('build_pipeline_model: schema rejects malformed actions', async () => {
  ...
  await assert.rejects(() => e.build_pipeline_model({ action: 'add_step', draft_id: 'ctx_deadbeef' }), 'add_step requires a stage');
...
test('build_pipeline_model: schema rejects action-irrelevant fields', async () => {
  ...
  await assert.rejects(() => e.build_pipeline_model({ action: 'add_step', draft_id: s.draft_id, stage: mr, source: 'events' }), 'add_step + source rejected');
  await assert.rejects(() => e.build_pipeline_model({ action: 'add_step', draft_id: s.draft_id, stage: mr, name: 'x' }), 'add_step + name rejected');
  // preview/commit/discard take only draft_id.
  await assert.rejects(() => e.build_pipeline_model({ action: 'preview', draft_id: s.draft_id, stage: mr }), 'preview + stage rejected');
  await assert.rejects(() => e.build_pipeline_model({ action: 'materialize', draft_id: s.draft_id, materialized: 'view' }), 'commit + materialized rejected');
  // start MAY carry draft_id (legitimate context reuse) — not rejected.
  const reuse = await e.build_pipeline_model({ action: 'start', draft_id: s.draft_id, name: 'reused', source: 'events' });
...
test('build_pipeline_model: array op on a non-array column is rejected at add_step', async () => {
  const e = engine();
...
test('build_pipeline_model: edit_step / insert_step / delete_step / truncate mutate in place', async () => {
  ...
  const d = s.draft_id;
  await e.build_pipeline_model({ action: 'add_step', draft_id: d, stage: { stage: 'where', conditions: [{ column: 'event_name', op: 'eq', value: 'first_launch' }] } });
  ...
  const ed = await e.build_pipeline_model({ action: 'edit_step', draft_id: d, index: 1, stage: { stage: 'where', conditions: [{ column: 'event_name', op: 'eq', value: 'tutorial' }] } });
  ...
  const ins = await e.build_pipeline_model({ action: 'insert_step', draft_id: d, index: 1, stage: { stage: 'where', conditions: [{ column: 'event_name', op: 'in', value: ['tutorial', 'first_launch'] }] } });
  assert.equal(ins.steps.length, 3);
  assert.equal(ins.steps[0].conditions[0].op, 'in', 'inserted at the front');
  // delete step 2.
  const del = await e.build_pipeline_model({ action: 'delete_step', draft_id: d, index: 2 });
  assert.equal(del.steps.length, 2);
  // truncate back to a single step (the cheap "go back to step N").
  const tr = await e.build_pipeline_model({ action: 'truncate', draft_id: d, after: 1 });
  assert.equal(tr.steps.length, 1);
  // out-of-range index/after are rejected.
  await assert.rejects(() => e.build_pipeline_model({ action: 'edit_step', draft_id: d, index: 9, stage: mr }), /out of range/);
  await assert.rejects(() => e.build_pipeline_model({ action: 'truncate', draft_id: d, after: 9 }), /out of range/);
});
...
test('build_pipeline_model: an edit breaking a downstream step reports the step index, draft intact', async () => {
  ...
  const d = s.draft_id;
  await e.build_pipeline_model({ action: 'add_step', draft_id: d, stage: mr }); // step 1: funnel exposes reached_a/completed
  await e.build_pipeline_model({ action: 'add_step', draft_id: d, stage: { stage: 'where', conditions: [{ column: 'completed', op: 'eq', value: true }] } }); // step 2 uses 'completed'
  // replacing step 1 with a plain where removes 'completed' → step 2 can no longer reference it.
  await assert.rejects(
    () => e.build_pipeline_model({ action: 'edit_step', draft_id: d, index: 1, stage: { stage: 'where', conditions: [{ column: 'event_name', op: 'eq', value: 'first_launch' }] } }),
    /step 2/,
  );
  const pv = await e.build_pipeline_model({ action: 'preview', draft_id: d });
...
test('build_pipeline_model: one_per_match funnel warns to filter completed', async () => {
  ...
  const a = await e.build_pipeline_model({ action: 'add_step', draft_id: s.draft_id, stage: { ...mr, rows: 'one_per_match' } });
  ...
  const a2 = await e.build_pipeline_model({ action: 'add_step', draft_id: s2.draft_id, stage: mr });
