...
| **Python on top of a pipeline (statistics, clustering, scoring)** | a final `python` stage in `build_native_model` / `register_native_model`: the SQL stages land as the prep TABLE `pipe_<name>_prep`, the stage becomes the dbt Python model `pipe_<name>` (`def model(dbt, session)`, `dbt.ref` to the prep) that dbt runs on the warehouse's Python runtime (BigQuery BigFrames/Dataproc, Snowpark, PySpark; DuckDB locally). Caller declares allowlisted imports, its own functions over a pandas frame and the ordered calls; bodies pass a static AST gate; `dbt.ref`/`dbt.config`/`return` are generated. Result read with `get_query_result` as any pipeline | **PROVEN on dbt-duckdb** (`python-stage.test.js`: per-player z-scores −0.1355/−1.1514/+1.2869, tiers low/low/high/low, runtime error surfaced from dbt) |
