...
const TOOL_DESCRIPTIONS = {
  ...
  describe_index: 'Report operational state: the value-index SYNC status (last/recent background refresh runs, seconds since last sync, coverage = indexed properties + stored values, whether a refresh is in flight) plus background query jobs and their statuses. Pass { property } to drill into one event property: its timing history plus null coverage — overall null_count/fraction and a per-event_name breakdown (row_count, non_null, null_count) annotated with whether the property applies to that event, so NULLs that are expected (the property is not populated on that event) are distinguishable from real data gaps. Pass { run } for one sync run\'s per-property breakdown. Read-only and cheap (no warehouse). Check it to know whether describe_catalog sample_values are fresh or still filling in, and to see what is currently running.',
