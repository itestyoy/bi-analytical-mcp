...
## Notes
- dbt runs in named environments — one virtualenv each under `/opt/dbt-envs` (`DBT_ENVS_DIR`), picked by `DBT_ENV`: `dbt-v2` is dbt v2 (build arg `INSTALL_DBT_V2=0` skips it — then set `DBT_ENV=dbt-v1`), `dbt-v1` is dbt 1.x + the adapter. MetricFlow is an environment of its own, `metricflow` (`MF_ENV` names another), which every dbt environment queries metrics through. The build installs each from its hash lock (`config/dbt-environments/`, via `scripts/dbt-env.mjs create`): exactly the packages `src/dbt/environment-specs.js` names, every file checked by SHA-256, wheels only — there is no requirements file to pass. The one build arg is `WAREHOUSE_ADAPTER` (`duckdb`, the default, or `bigquery`), the adapter `dbt-v1` and `metricflow` are built with. The locks are for Python 3.11 on x86_64 Linux, so on an arm64 host build with `--platform linux/amd64` (or add that platform's locks: `LOCK_PLATFORM` in the specs, then `npm run dbt:env -- lock`). `docker-compose.yml` runs `dbt-v2`, the BigQuery setup `dbt-v1` until the python stage is proven on v2 there. The server reads the dbt version from the binary (`DBT_VERSION` pins it; `DBT_BIN`/`MF_BIN` name binaries outright). On v2 the semantic layer is written in dbt's latest YAML spec, and the python stage is not offered on DuckDB. v2 downloads its ADBC driver from dbt's CDN on the first run — allow that once, or warm it at build time.
