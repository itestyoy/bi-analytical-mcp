...
const TOOL_DESCRIPTIONS = {
  ...
  build_native_model: 'The ESCAPE HATCH for a one-off derived TABLE whose rows ARE the answer — funnels (match_recognize), sessionization, window functions, pivots, anything the governed metrics cannot express. Composed INCREMENTALLY (single `action`-driven tool): start a draft, add_step one stage at a time (where/derive/compute/unnest/join/aggregate/pivot/unpivot/window/order_by/limit + match_recognize; a join names the RELATIONSHIP the schema declares (via: <name>) and never its columns, and joins STACK so one pipeline can reach several sources) — each add_step validates the stage and returns the columns then available for the NEXT stage (pure schema, NOTHING materialized until materialize) — optionally preview the SQL, then materialize (builds + runs the model). The rows are read back with get_query_result (NOT query_semantic_model). For REUSABLE named metrics you query many ways, prefer create_semantic_model (the governed path). A `python` stage (imports + your functions + ordered calls over the frame dbt.ref() returns — BigFrames/Snowpark/PySpark, untouched) is a dbt PYTHON model of its own, allowed anywhere and repeatedly — and it carries ONLY what SQL cannot say (a statistical test, clustering, scoring, a forecast): the table it reads is prepared by the SQL stages before it (scope, payload columns, joins, the aggregation to the analysis grain), never the raw source. BEFORE writing one: read semantic_index({ guide: "python" }) for this warehouse\'s frame rules, and STUDY the worked recipes — the stage description indexes them by the MOVE each one covers, and semantic_index({ recipe: "<id>" }) returns one in full; fetch every move your function needs, because a compiling payload adapted beats a stage written from scratch: the pipeline becomes a chain of dbt models reading each other via ref, and dbt runs the Python ones on the warehouse\'s Python runtime (never here) — for statistics, clustering, scoring; its table is read with get_query_result like any pipeline.',
...
2. create_semantic_model — declare measures/dimensions/metrics for a task in an ISOLATED context (returns a context_id). Pass that context_id back to extend the same context.
   ...
   - Beyond SQL (z-scores, clustering, scoring, forecasting), where the overview's python_models says available — and ONLY the part SQL cannot express: prepare the table in SQL stages first (scope by event and time, extract the payload columns, join the attributes, aggregate to the grain the analysis works on), then hand THAT to python: START by reading semantic_index({ guide: "python" }) (the frame rules of this warehouse + the right form per operation) and the recipes for the moves your function needs — the stage description lists each id with the move it covers, semantic_index({ recipe: "<id>" }) returns one in full; then add a 'python' stage to that pipeline (anywhere, even several — each is a dbt Python model of its own in a chain dbt builds in order; the stage before it lands as a table it reads, or it reads the source when first) — declare allowlisted imports, your own functions (def f(df, …): … return df) over the frame dbt.ref() returns on this warehouse (BigFrames / Snowpark / PySpark — the stage description names it; nothing is converted for you) and the ordered calls; the server writes the dbt Python model (dbt.ref/dbt.config/return) and dbt runs it on the warehouse's Python runtime. Read the result with get_query_result as usual.
