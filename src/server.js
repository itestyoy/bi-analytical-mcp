...
- measures sources (optional): a NON-events fact whose columns are amounts rather than events (e.g. acquisition spend at one row per player x day). It has no event_name; it declares its own time axis and its own measures IN THE CATALOG, so semantic_index({ model }) lists them with their aggregation, expression and unit, and you reference those names in create_semantic_model metrics without declaring measures of your own. It carries the user entity, so user__<attr> segments it too; in a pipeline, join it on a COMPOSITE key (player + day) so a daily grain does not fan out.
