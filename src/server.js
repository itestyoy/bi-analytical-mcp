...
const TOOL_DESCRIPTIONS = {
  ...
  memory: 'DURABLE analyst memory — remember what you FOUND OUT so it comes back through semantic_index. After you resolve something non-obvious (a vague request tracked down to a real field, a gotcha, a useful source), record:"action" it: `note` the finding, `targets` the catalog entities it is about (a property/attribute/event/model — e.g. "ad_type_of_event_data", "users.country", "ad_finished"), `aliases` the words the user actually used ("ad format"), `links` any sources. The note then surfaces inline on the linked semantic_index views ({ model }/{ event }/{ property }) and in semantic_index({ search }) — so the next fuzzy phrasing resolves straight to the right field instead of re-investigating. action: list (all, or one { target }) | search (by word) | forget (by id).',
