...
test('materialize keeps the draft and records the built table as a checkpoint; the next step reads it', async () => {
  const e = engine();
  const { draft_id } = await e.build_native_model({ action: 'start', name: 'seg', source: 'events' });
  await e.build_native_model({ action: 'add_step', draft_id, stage: agg('events_seen') });
  const built = await e.build_native_model({ action: 'materialize', draft_id });
  ...
  const next = await e.build_native_model({ action: 'add_step', draft_id, stage: { stage: 'where', conditions: [{ column: 'events_seen', op: 'gte', value: 2 }] } });
  ...
  const again = await e.build_native_model({ action: 'materialize', draft_id });
  ...
  await assert.rejects(() => e.build_native_model({ action: 'materialize', draft_id }), /nothing to build/);
});
...
test('invalidation is positional: an edit retires the checkpoints whose prefix contains that step', async () => {
  ...
  const { draft_id } = await e.build_native_model({ action: 'start', name: 'seg', source: 'events' });
  await e.build_native_model({ action: 'add_step', draft_id, stage: agg('events_seen') });
  await e.build_native_model({ action: 'add_step', draft_id, stage: { stage: 'where', conditions: [{ column: 'events_seen', op: 'gte', value: 2 }] } });
  const c1 = await e.build_native_model({ action: 'materialize', draft_id }); // at: 2
  await e.build_native_model({ action: 'add_step', draft_id, stage: { stage: 'compute', name: 'twice', op: 'mul', left: { column: 'events_seen' }, right: { value: 2 } } });
  await e.build_native_model({ action: 'add_step', draft_id, stage: { stage: 'order_by', keys: [{ key: 'twice', direction: 'desc' }] } });
  const c2 = await e.build_native_model({ action: 'materialize', draft_id }); // at: 4
  ...
  const edit = await e.build_native_model({ action: 'edit_step', draft_id, index: 3, stage: { stage: 'compute', name: 'twice', op: 'mul', left: { column: 'events_seen' }, right: { value: 3 } } });
  ...
  const deep = await e.build_native_model({ action: 'edit_step', draft_id, index: 1, stage: agg('events_seen') });
...
test('truncate retires the checkpoints past the kept prefix; add_step never retires one', async () => {
  ...
  const { draft_id } = await e.build_native_model({ action: 'start', name: 'seg', source: 'events' });
  await e.build_native_model({ action: 'add_step', draft_id, stage: agg('events_seen') });
  await e.build_native_model({ action: 'materialize', draft_id }); // at: 1
  await e.build_native_model({ action: 'add_step', draft_id, stage: { stage: 'where', conditions: [{ column: 'events_seen', op: 'gte', value: 2 }] } });
  await e.build_native_model({ action: 'materialize', draft_id }); // at: 2
  const added = await e.build_native_model({ action: 'add_step', draft_id, stage: { stage: 'project', columns: ['player_id_of_internal'] } });
  assert.equal(added.checkpoints_dropped, undefined, 'appending never invalidates a prefix');
  const tr = await e.build_native_model({ action: 'truncate', draft_id, after: 1 });
...
test('a stage needing the source columns is refused after a checkpoint that dropped them, accepted after one that kept them', async () => {
  ...
  const a = await e.build_native_model({ action: 'start', name: 'seg', source: 'events' });
  await e.build_native_model({ action: 'add_step', draft_id: a.draft_id, stage: agg('events_seen') });
  await e.build_native_model({ action: 'materialize', draft_id: a.draft_id });
  await assert.rejects(
    () => e.build_native_model({ action: 'add_step', draft_id: a.draft_id, stage: funnel }),
    /no longer that source's events/,
  );
  // a payload read is refused for the same reason — the column it reads is not there.
  await assert.rejects(
    () => e.build_native_model({ action: 'add_step', draft_id: a.draft_id, stage: { stage: 'derive', name: 'lvl', op: 'extract', source: 'level_id_of_event_data' } }),
    /unknown column/,
  );

  // (b) checkpoint where the rows are still events (a where) → the funnel is legal on top of it.
  const b = await e.build_native_model({ action: 'start', name: 'funnel_on_slice', source: 'events' });
  await e.build_native_model({ action: 'add_step', draft_id: b.draft_id, stage: keepEvents('level_started') });
  const built = await e.build_native_model({ action: 'materialize', draft_id: b.draft_id });
  assert.equal(built.checkpoint.carries_source, 'events');
  const ok = await e.build_native_model({ action: 'add_step', draft_id: b.draft_id, stage: funnel, include_columns: true });
...
test('fork inherits a checkpoint it keeps, copies its definition, and the owner cannot be dropped under it', async () => {
  ...
  const { draft_id } = await e.build_native_model({ action: 'start', name: 'seg', source: 'events' });
  await e.build_native_model({ action: 'add_step', draft_id, stage: agg('events_seen') });
  const c1 = await e.build_native_model({ action: 'materialize', draft_id }); // at: 1
  await e.build_native_model({ action: 'add_step', draft_id, stage: { stage: 'where', conditions: [{ column: 'events_seen', op: 'gte', value: 5 }] } });
  const c2 = await e.build_native_model({ action: 'materialize', draft_id }); // at: 2

  const fork = await e.build_native_model({ action: 'fork', draft_id, after: 1, name: 'seg_alt' });
  ...
  const step = await e.build_native_model({ action: 'add_step', draft_id: fork.draft_id, stage: { stage: 'where', conditions: [{ column: 'events_seen', op: 'lt', value: 5 }] } });
...
test('a checkpoint is retired when the value index moved on, or its model is gone', async () => {
  ...
  const afterScan = await e.build_native_model({ action: 'add_step', draft_id, stage: { stage: 'project', columns: ['player_id_of_internal'] } });
  ...
  const second = await e.build_native_model({ action: 'materialize', draft_id });
  e.ctxs.removePipelineFiles(draft_id, second.model);
  const afterDelete = await e.build_native_model({ action: 'add_step', draft_id, stage: { stage: 'project', columns: ['player_id_of_internal'] } });
...
test('a new draft in the same context never reuses a model name', async () => {
  const e = engine();
  const first = await e.build_native_model({ action: 'start', name: 'seg', source: 'events' });
  await e.build_native_model({ action: 'add_step', draft_id: first.draft_id, stage: agg('events_seen') });
  const built = await e.build_native_model({ action: 'materialize', draft_id: first.draft_id });
  const fork = await e.build_native_model({ action: 'fork', draft_id: first.draft_id, after: 1, name: 'seg_alt' });
  ...
  const again = await e.build_native_model({ action: 'start', name: 'seg', source: 'events', draft_id: first.draft_id });
  assert.equal(again.draft_id, first.draft_id);
  await e.build_native_model({ action: 'add_step', draft_id: first.draft_id, stage: agg('other_count') });
  const rebuilt = await e.build_native_model({ action: 'materialize', draft_id: first.draft_id });
...
test('the idle GC does not reclaim a context whose prefix a fork reads', async () => {
  ...
  const { draft_id } = await e.build_native_model({ action: 'start', name: 'seg', source: 'events' });
  await e.build_native_model({ action: 'add_step', draft_id, stage: agg('events_seen') });
  await e.build_native_model({ action: 'materialize', draft_id });
  const fork = await e.build_native_model({ action: 'fork', draft_id, after: 1, name: 'seg_alt' });
