...
const TOOL_DESCRIPTIONS = {
  ...
  time: `Wait for \`seconds\` (capped at ${MAX_WAIT_SECONDS}), then return — a timer that touches no data and follows no task. To wait for a task, call its query tool with { task_id } — query_semantic_model or query_pipeline_model: it returns the moment the task is done.`,
};
...
4. TASKS — every call that does warehouse work (build_semantic_model, query_semantic_model, build_pipeline_model materialize, query_pipeline_model) returns ONLY a task_id and never waits. The query tool of the same side reads it back: query_semantic_model({ task_id }) for the semantic side (a model being parsed, a metric query), query_pipeline_model({ task_id }) for the pipeline side (a build, a query over a built model) — it waits (up to ${MAX_WAIT_SECONDS}s per call; again while it says running) and returns the result; offset/limit page a stored one. Independent queries on one context go in one call (queries: up to ${MAX_BATCH}, run side by side) and are read together ({ task_ids }). To build further on a stored result without recomputing it, start a pipeline from it: build_pipeline_model({ action: 'start', name, from_task }).
...
- when in doubt which builder: build_semantic_model = reusable named metrics (query many ways); build_pipeline_model = a one-off derived table (funnel/sessionization/window/pivot), queried with query_pipeline_model.`;
