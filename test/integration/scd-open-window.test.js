...
test('a point-in-time join keeps the open-ended current version, and still picks the right one per event', async (t) => {
  ...
  const sums = await wh.query(`SELECT country, sum(amount)::int AS total FROM (${sql}) x GROUP BY country ORDER BY country`);
  ...
  const fanout = await wh.query(`SELECT count(*)::int AS n FROM (${sql}) x`);
