...
- dbt RUNS IN NAMED ENVIRONMENTS (`src/dbt/environments.js`): a virtualenv per environment under
  ...
  binary. `npm run dbt:env -- create|verify|list` manages them.
- WHAT IS IN AN ENVIRONMENT IS THIS TOOL'S DECISION (HARD RULE): `src/dbt/environment-specs.js` names
  each one's exact packages (per adapter where it matters), `config/dbt-environments/*.lock.txt` —
  generated by `npm run dbt:env -- lock`, never edited — pins every transitive package with the
  SHA-256 of its files, and `create` installs only that (a locked pip, `--require-hashes`,
  `--only-binary :all:`, `--no-deps`; a `sourceBuilds` package from its hashed sdist with the locked
  build tools and no build isolation), removes what the venv brought beyond it, and `verify`s.
  Do NOT add a requirements file, a `pip install <pkg>` in the Dockerfile, or an option to hand the
  tool packages of one's own: a version change is a spec change + re-lock, reviewed as code.
