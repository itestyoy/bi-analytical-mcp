  ...
  if (waits) {
    ...
    try {
      while (engine.jobs.get(args.task_id)?.status === 'running' && engine.jobs.isLive?.(args.task_id)) {
        ...
        await withSignal(signal, () => engine._awaitTask(args.task_id, MAX_WAIT_SECONDS));
      }
    } catch (err) {
      const cancelled = !!signal?.aborted;
