...
test('require_time_range rejects an unbounded pipeline; a bounded one passes validation', async () => {
  ...
  await assert.rejects(() => e.build_native_model({ action: 'commit', draft_id: s.draft_id }), /require_time_range/);
  ...
  const out = await e.build_native_model({ action: 'commit', draft_id: s2.draft_id });
  ...
  const out3 = await e.build_native_model({ action: 'commit', draft_id: s3.draft_id });
...
test('without require_time_range an unbounded pipeline is not rejected', async () => {
  ...
  await e.build_native_model({ action: 'add_step', draft_id: s.draft_id, stage: { stage: 'aggregate', group_by: ['event_name'], measures: [{ name: 'n', fn: 'count' }] } });
  const out = await e.build_native_model({ action: 'commit', draft_id: s.draft_id });
