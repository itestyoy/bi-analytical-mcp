...
| **Every shipped recipe builds & runs** | `semantic_index({ recipe: id }) → create_semantic_model → query` (a `register_payload` one builds and runs; a `tool_calls` one computes; a `requires: python_models` one compiles under `dry_run`) | **PROVEN** (`recipes-parse.test.js` covers every entry of `config/recipes.json`) |
